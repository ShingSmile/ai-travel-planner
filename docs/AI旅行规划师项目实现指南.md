...
> **执行进度提醒**：任务 1-9 已完成（最新完成任务 9：行程生成流程联通），下一步将进入任务 10。
...
10. **行程详情页面**
    ...
    - 实现基础活动编辑（时间、备注、删除）。
11. **高德地图集成**
    - 接入高德 JS SDK，绘制行程点位与路线。
    - 实现活动卡片与地图点位联动、交通方式展示。
12. **POI 数据完善**
    - 后端调用高德 Web 服务（POI 搜索）补充经纬度、图片。
    - 缓存 POI 结果（Supabase Edge Function 或 Redis 替代实现）。
13. **预算估算与展示**
    - 从 LLM 结果解析预算数据，存入 `trips.budget_breakdown`。
    - 前端新增预算组件（分类摘要、预算剩余提醒）。
14. **费用记录功能**
    - `POST /api/expenses`、`GET /api/expenses` 实现 CRUD。
    - 前端构建费用表格、分类筛选、支出图表。
15. **语音录制前端**
